...
class GeneradorAbstractPalette:
    ...
    def generar_paleta(self):
        if not self.color_principal:
            ...
            color_hsl_principal = (
                h * 360,  # Hue
                max(40, min(100, s * 100)),  # Saturation 
                max(30, min(85, l * 100))   # Brightness
            )

        # Seleccionar jerarquía
        jerarquias = ['diagonal', 'espiral', 'simetria', 'tercios']
        probabilidades = [0.28, 0.27, 0.31, 0.14]
        ...
        # Rango de tonos
        rango_tonos = [(0, 60), (310, 360)] if self.tipo_paleta == 'Calida' else [(180, 300)]

        # Calcular direcciones
        direccion_saturacion = self.ajustar_direccion(total_saturacion)
        direccion_brightness = self.ajustar_direccion(total_brightness)

        colores_validos = []

        for i in range(1, cantidad_colores + 1):
            nuevo_hue = (color_hsl_principal[0] + (-1 if self.tipo_paleta == 'Calida' else 1) * diferencia_tono * i) % 360

            if any(rango[0] <= nuevo_hue <= rango[1] for rango in rango_tonos):
                nuevo_saturation = max(40, min(100, color_hsl_principal[1] + direccion_saturacion * total_saturacion * (i / cantidad_colores)))
                nuevo_brightness = max(50, min(90, color_hsl_principal[2] + direccion_brightness * total_brightness * (i / cantidad_colores)))

                # Evitar colores lavados
                if nuevo_brightness > 90 and nuevo_saturation < 30:
                    continue
                ...
                for color_anterior in colores_validos:
                    if abs(nuevo_color[0] - color_anterior[0]) < 20 and \
                    abs(nuevo_color[1] - color_anterior[1]) < 20 and \
                    abs(nuevo_color[2] - color_anterior[2]) < 20:
                        es_valido = False
                        break
...
if __name__ == '__main__':
    generador = GeneradorAbstractPalette("paso2_probabilidades/probabilidadesColorPrincipal_Abstracto.csv", "paso2_probabilidades/probabilidadesColorDiferencial_Abstracto.csv")
